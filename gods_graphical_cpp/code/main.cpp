...
static Game_State quick_setup(std::optional<int> seed) {
  ...
  Player p0; p0.name = "Player 1";
  Player p1; p1.name = "Player 2";
  game.players  = {p0, p1};
  game.peoples  = {};
...
static Table_State init_table_state(Game_State& gods_state, UI_State& ui_state,
                                    int bottom_player = 0) {
...
static void draw_hud(Table_State* table_state, Game_State& gods_state,
                     const std::optional<Choice>& current_choice,
                     UI_State& ui_state, int bottom_player) {
  int H      = kt::WINDOW_HEIGHT;
  int h      = kt::CARD_HEIGHT;
  int margin = 20;
  KT_Rectangle window = {0.0f, 0.0f, (float)kt::WINDOW_WIDTH, (float)H};
  int bottom_wonders_y = (int)place_inside(window, 0, h, "left", "bottom", 2 * margin + h).y;
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    draw_player_hud(i, score, (int)gods_state.players[i].deck.size(), is_current, hud_y);
  }
  ...
  std::string label = ui_state.playground ? "Playground: ON" : "Playground: OFF";
  ...
  if (card_id != -1 && ui_state.playground) {
    ...
    KT_Rectangle card_rect = {kt_card.x, kt_card.y, (float)kt::CARD_WIDTH, (float)kt::CARD_HEIGHT};
    KT_Rectangle panel = place_next(card_rect, panel_w, btn_h + 16, "center", "bottom", 8);
    panel.x = std::max(0.0f, std::min(panel.x, (float)(kt::WINDOW_WIDTH - panel_w)));
    panel.y = std::max(0.0f, std::min(panel.y, (float)(kt::WINDOW_HEIGHT - (int)panel.height)));
    DrawRectangleRounded(
      Rectangle{panel.x, panel.y, panel.width, panel.height},
      0.3f, 8, ::Color{20, 20, 20, 200}
    );
  ...
  for (Stack& s : table_state->stacks) {
    if (s.name == "shared_deck") {
      KT_Rectangle target = ui_state.playground
        ? place_inside(window, kt::CARD_WIDTH, kt::CARD_HEIGHT, "right", "center", 10)
        : place_next(window, kt::CARD_WIDTH, kt::CARD_HEIGHT, "right", "center", 10);
      s.rect = target;
...
static void play_gods(Game_State& gods_state, Table_State& table_state, UI_State& ui_state,
                     Agent* agent, int player_index,
                     UDP_Socket* sock, std::pair<std::string, int> friend_addr) {
  if (!IsWindowReady()) {
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsKeyDown(KEY_SPACE)) {
      auto r = find_card_at((float)GetMouseX(), (float)GetMouseY(), table_state);
    ...
    if (ui_state.power_edit_card_id != -1) {
      int keys[10] = {KEY_ONE, KEY_TWO, KEY_THREE, KEY_FOUR, KEY_FIVE,
                      KEY_SIX, KEY_SEVEN, KEY_EIGHT, KEY_NINE, KEY_ZERO};
      for (int i = 0; i < 10; ++i) {
        if (IsKeyPressed(keys[i])) {
          gods_state.all_cards[ui_state.power_edit_card_id].power = (i < 9) ? (i + 1) : 10;
          ui_state.power_edit_card_id = -1;
          gods_state.notify_cards_changed();
          break;
        }
    ...
    for (int i = 0; i < (int)table_state.stacks.size(); ++i) {
      if (table_state.stacks[i].name == "p" + std::to_string(player_index) + "_discard") discard_you = i;
      if (table_state.stacks[i].name == "p" + std::to_string(1 - player_index) + "_discard") discard_opponent = i;
    }
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      for (int stack_id : {discard_opponent, discard_you}) {
        ...
        if (inside && !is_expanded) {
          s.rect = ui_state.place(kt::CARD_WIDTH * 7, kt::CARD_HEIGHT, "center", "center");
          s.spread_x = 150.0f;
          s.depth    = 1.0f;
          update_card_positions(s, table_state, false);
    ...
    if ((!agent || ui_state.playground) && sock) {
      ...
      bool should_send = dropped.has_value()
                         || IsKeyPressed(KEY_R) || IsKeyPressed(KEY_S);
      if (should_send) {
        nlohmann::json stacks = nlohmann::json::array();
        for (const Stack& s : table_state.stacks) stacks.push_back(s.cards);
        nlohmann::json sm;
    ...
    if (sock) {
      ...
      if (msg_opt) {
        ...
        if (t == "stacks") {
          ...
          for (size_t i = 0; i < arr.size() && i < table_state.stacks.size(); ++i) {
  ...
  if (gods_state.game_over) {
    ...
    std::vector<std::string> names = {gods_state.players[0].name, gods_state.players[1].name};
    std::string result_text =
      scores[player_index] > scores[1 - player_index] ? "You win!" :
      scores[player_index] < scores[1 - player_index] ? "You lose!" : "It's a tie!";
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    std::string a = argv[i];
    if (a == "agent") skip_menu_vs_ai = true;
  ...
  UI_State    ui_state;
  Table_State table_state = init_table_state(gods_state, ui_state, player_index);
  ...
  if (sock) {
    wrap_local      = std::make_unique<Agent_Local_Online>(&agent_ui, sock, friend_addr);
  ...
  play_gods(gods_state, table_state, ui_state, &duel, player_index, sock, friend_addr);
