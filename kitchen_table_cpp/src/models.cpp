...
// Proxy types that hold a raw pointer to a vector and a Python reference to the owner,
// giving true element reference semantics from Python without copying on __getitem__.
struct Stacks_Ref { std::vector<Stack>* vec; nb::object owner; };
struct Cards_Ref  { std::vector<Card>*  vec; nb::object owner; };
...
void bind_models(nb::module_& m) {
    ...
    // cards is exposed as a by-reference IntVector so Python mutations are visible in C++.
    ...
    // CardVector proxy — __getitem__ returns by copy (read-only access is sufficient since
    // Python does not mutate individual card fields; C++ handles card position updates).
    nb::class_<Cards_Ref>(m, "CardVector")
        .def("__len__", [](const Cards_Ref& r) { return r.vec->size(); })
        .def("__getitem__", [](Cards_Ref& r, int i) -> Card& {
            int n = (int)r.vec->size();
            if (i < 0) i += n;
            if (i < 0 || i >= n) throw nb::index_error("index out of range");
            return (*r.vec)[i];
        }, nb::rv_policy::reference_internal)
        .def("__setitem__", [](Cards_Ref& r, int i, const Card& v) {
            int n = (int)r.vec->size();
            if (i < 0) i += n;
            (*r.vec)[i] = v;
        })
    ...
    // StackVector proxy — delegates __getitem__ to owner._get_stack() which returns a true Stack&
    // reference with Table_State as parent, giving correct nanobind reference semantics.
    nb::class_<Stacks_Ref>(m, "StackVector")
        .def("__len__", [](const Stacks_Ref& r) { return r.vec->size(); })
        .def("__getitem__", [](Stacks_Ref& r, int i) {
            int n = (int)r.vec->size();
            if (i < 0) i += n;
            if (i < 0 || i >= n) throw nb::index_error("index out of range");
            // Delegate to Table_State._get_stack so the returned Stack& is owned by Table_State.
            return r.owner.attr("get_stack")(i);
        })
        .def("__setitem__", [](Stacks_Ref& r, int i, const Stack& v) {
            int n = (int)r.vec->size();
            if (i < 0) i += n;
            (*r.vec)[i] = v;
        })
    ...
    nb::class_<Table_State>(m, "Table_State")
        ...
        .def("__init__", [](Table_State* t,
            ...
            for (auto item : loose_cards) t->loose_cards.push_back(nb::cast<int>(item));
            if (draw_callback)        t->draw_callback        = std::move(draw_callback);
            if (is_drop_card_allowed) t->is_drop_card_allowed = std::move(is_drop_card_allowed);
           ...
           "zoomed_card_id"_a      = -1)
        // Proxy getters store a Python reference to self so element references stay valid.
        .def_prop_rw("cards",
            [](nb::object self) {
        ...
        // Used by the StackVector proxy's __getitem__ to return a true Stack reference.
        .def("get_stack", [](Table_State& t, int i) -> Stack& { return t.stacks[i]; },
             nb::rv_policy::reference_internal)
