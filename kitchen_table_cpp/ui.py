...
from kitchen_table_cpp.rendering import color_from_tuple, render_text, text_width
from kitchen_table_cpp.models import Table_State
...
def place_next(
    rect: Rectangle, width: int, height: int, x: str, y: str, padding: int = 0
) -> Rectangle:
...
def immediate_button(
    rectangle: Rectangle, label: str, color: Color = None, text_color: Color = None
) -> bool:
...
def immediate_buttons(
    size: tuple[int, int],
    buttons: list[tuple[tuple[int, int], str]],
    color: Color = None,
...
class UI_State:
    ...
    def place(
        self, width: int, height: int, x: str = "left", y: str = "top", padding: int = 0
    ) -> Rectangle:
