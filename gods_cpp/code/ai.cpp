...
float Agent_Minimax_Stochastic_Gods::evaluate_state(Game_State& game, int player_index) {
  ...
  // Terminal: large reward for win/loss; tie favors the inactive player (whoever
  // didn't move into the tied state).
  int my_score  = compute_player_score(game, player_index);
  int opp_score = compute_player_score(game, 1 - player_index);
...
Game_State sample_state(const Game_State& state, int player_index, std::mt19937& rng) {
...
int Agent_Minimax_Stochastic_Gods::choose_action(Game& state, const Choice& choice) {
...
void bind_agent(nb::module_& m) {
  nb::class_<Agent_Minimax_Stochastic_Gods>(m, "Agent_Minimax_Stochastic_Gods")
    ...
    .def("message",
         [](Agent_Minimax_Stochastic_Gods&, const std::string&) {})
    .def("choose_action",
