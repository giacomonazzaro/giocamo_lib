...
static Move agent_move(Game_State& game, int depth) {
  ...
  game.begin_game(game.next_choice());  // The position is built by hand here.
...
int main() {
  ...
  // Case 1: white rook a1 can take an undefended black queen on a8.
  {
    ...
    all_ok  = all_ok && ok;
    std::printf("case 1 (win free queen): Ra1xa8 expected, got %d->%d  %s\n",
                move.from, move.to, ok ? "OK" : "FAILED");
  }
  ...
  // Ra1-a8 is mate.
  {
    ...
    bool ok = move.from == 0 && move.to == 56;
    all_ok  = all_ok && ok;
    std::printf("case 2 (mate in 1): Ra1-a8# expected, got %d->%d  %s\n",
                move.from, move.to, ok ? "OK" : "FAILED");
  }
