...
from pyray import *
from kitchen_table_cpp.models import Card, Drag_State, Stack, Table_State
from kitchen_table_cpp.config import tweak
import kitchen_table_cpp.game_state as gs
...
def point_in_stack_area(px: float, py: float, stack: Stack) -> bool:
    ...
    def point_in_rect(
        mx: float, my: float, x: float, y: float, w: float, h: float
    ) -> bool:
...
def handle_mouse_release(state: Table_State) -> None:
    ...
    if not state.is_drop_card_allowed(
...
def handle_mouse_move(state: Table_State) -> None:
    ...
    if drag.last_hovered_stack != hovered_stack:
        if (
            drag.current_stack >= 0
            and drag.card_id in state.stacks[drag.current_stack].cards
        ):
        ...
        if state.is_drop_card_allowed(
            drag.original_stack, hovered_stack, drag.card_id
        ) and not stack_is_full(state.stacks[hovered_stack]):
...
def update_input(state: Table_State) -> None:
    ...
    if is_key_pressed(KeyboardKey.KEY_R):
        if is_key_down(KeyboardKey.KEY_LEFT_SHIFT) or is_key_down(
