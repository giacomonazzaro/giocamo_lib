...
def draw_background(turn: float = 0.0):
    ...
    rl.SetShaderValue(
        _background_shader, _background_time_loc, t, rl.SHADER_UNIFORM_FLOAT
    )

    res = rl.ffi.new("float[2]", [get_screen_width(), get_screen_height()])
    rl.SetShaderValue(
        _background_shader, _background_resolution_loc, res, rl.SHADER_UNIFORM_VEC2
    )

    turn_val = rl.ffi.new("float *", _background_turn_value)
    rl.SetShaderValue(
        _background_shader, _background_turn_loc, turn_val, rl.SHADER_UNIFORM_FLOAT
    )
...
def render_text(text: str, x: int, y: int, size: int, color: Color) -> None:
    ...
    draw_text_ex(
        get_font(), text, Vector2(x, y), float(size), tweak["font_spacing"], color
    )
...
def draw_card_back() -> None:
    ...
    draw_rectangle_rounded(
        Rectangle(x, y, w, h), r / min(w, h), 8, color_from_tuple(tweak["card_back"])
    )
...
def draw_stack_placeholder(stack: Stack) -> None:
    ...
    draw_rectangle_rounded_lines_ex(
        stack.rect, r / min(w, h), 8, 1, Color(100, 100, 100, 100)
    )
