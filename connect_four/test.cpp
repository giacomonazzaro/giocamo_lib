// Headless match: Agent_Minimax vs Agent_MCTS for Connect Four. Plays N games,
// alternating which seat minimax holds, and prints per-game results plus an
// aggregate win count and per-agent wall-clock totals so the two can be
// compared at equal time by tuning each side's knobs.
//
// Connect Four is perfect-information, so both agents are the non-stochastic
// variants and share the same terminal evaluate_state.
...
int main(int argc, char** argv) {
  int num_games           = 20;
  int minimax_depth       = 7;
  int mcts_iterations     = 20000;
  int mcts_rollout_depth  = 64;
  ...
  std::cerr << "connect_four_match_mcts: games=" << num_games
  ...
  Agent_Minimax<connect_four::Game_State> minimax_agent(minimax_depth);
  Agent_MCTS<connect_four::Game_State>    mcts_agent(
    mcts_iterations,
    mcts_rollout_depth,
  ...
  for (int game_index = 0; game_index < num_games; ++game_index) {
    ...
    // Agent_Duel.swap reorders [agent_0, agent_1]: swap=true puts the first
    // argument in seat 1 and the second in seat 0.
    ...
    int score_player_1 = connect_four::compute_player_score(state, 1);
