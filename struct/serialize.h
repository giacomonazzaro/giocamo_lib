...
struct has_custom_serialization<
  T,
  std::void_t<decltype(sizeof(Custom_Serialization<T>))>> : std::true_type {};
...
template <typename T>
inline void save_struct(const T& t, const std::string& path) {
  auto srl = make_writer(path, 1048576);
  assert(srl.file);
  ::serialize_struct(srl, const_cast<T&>(t));
  close_serializer(srl);
}

template <typename T>
inline T load_struct(const std::string& path) {
  auto t   = T{};
  auto srl = make_reader(path, 1048576);
  assert(srl.file);
  ::serialize_struct(srl, t);
  close_serializer(srl);
  return t;
}