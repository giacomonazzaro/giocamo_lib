...
std::string to_json(const std::vector<T, Alloc>& arr, int indent = 0,
                    bool pretty = true);
...
template <typename T>
auto from_json_impl(JsonParser& p, T& out)
...
// array<T> overload
template <typename T>
std::string to_json(const array<T>& arr, int indent = 0, bool pretty = true) {
    return array_to_json(arr, indent, pretty);
}
