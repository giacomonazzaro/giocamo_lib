...
void Reliable_UDP_State::send_ack(
  const std::string& msg_id, const std::string& ip, int port
) {
...
void Reliable_UDP_State::receiver_loop() {
  ...
  while (running) {
    ...
    ssize_t     n          = ::recvfrom(
      fd, buf, sizeof(buf), 0, reinterpret_cast<sockaddr*>(&sender), &sender_len
    );
    ...
    for (auto it = pending_acks.begin(); it != pending_acks.end();) {
      ...
      if (now - msg.send_time > RETRY_INTERVAL) {
        if (msg.retries < MAX_RETRIES) {
          ...
          ::sendto(
            fd,
            msg.packet_bytes.c_str(),
            msg.packet_bytes.size(),
            0,
            reinterpret_cast<sockaddr*>(&addr),
            sizeof(addr)
          );
...
void send_message(
  ...
  std::string packet = "{\"t\":\"DATA\",\"i\":\"" + msg_id +
                       "\",\"p\":" + payload + "}";
