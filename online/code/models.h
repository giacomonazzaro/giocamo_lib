...
struct Reliable_UDP_State {
  ...
  std::map<std::string, Pending_Msg>
                        pending_acks;  // msg_id -> pending message.
...
// Tracks the async state of a P2P connection setup, polled by the UI each
// frame.
struct Connection_State {
  ...
  int                         friend_port = 0;
  std::string                 error;
  std::mutex state_lock;  // Guards error / friend_ip / friend_port /
                          // player_index / seed.
